...
export class CalendarSetup extends React.Component<any, any> {
  ...
  getCmsSections(batchId: any) {
    academicSettingsServices.getCmsSections(batchId).then((response) => {
      this.setState({
        sections: response,
      });
    });
  ...
  componentDidMount() {
    academicSettingsServices.getGlobalConfiguration('admin').then(
      (response) => {
        ...
        if (
          response.selectedAcademicYearId === null ||
          response.selectedAcademicYearId === 0
        ) {
        ...
        if (academicYearId) {
          ...
          academicSettingsServices.getCmsTerms(academicYearId).then((response) => {
            this.setState({
              terms: response,
            });
          });
          ...
          academicSettingsServices.getCmsBatches(departmentId).then((response) => {
            this.setState({
              batches: response,
            });
          });
          academicSettingsServices.getCmsSubjects(departmentId).then((response) => {
            this.setState({
              subjects: response,
            });
          });
          academicSettingsServices.getCmsTeachers(departmentId).then((response) => {
            this.setState({
              teachers: response,
            });
          });
          academicSettingsServices
            .getFilterAttendanceMasterByDepartment(departmentId)
            .then((response) => {
              this.setState({
  ...
  createCalendarEvents(response: any) {
    ...
    for (let i = 0; i < response.length; i++) {
      ...
      let title =
        'Techer: ' +
        attendancemaster.teach.teacher.teacherName +
        ' Subject: ' +
        attendancemaster.teach.subject.subjectCode;
      if (!minTime) {
        minTime = new Date();
        minTime.setHours(
          startTime.getHours(),
      ...
      events.push({
        ...
        teacher:
          attendancemaster.teach.teacher.teacherName +
          ' ' +
          attendancemaster.teach.teacher.teacherLastName,
  ...
  onClickEvent(event: any, e: any) {
    console.log('1.selected events print:', event);
    let custEventValue: any = '';
    custEventValue = event;
    custEventValue.start = event.start.toLocaleString();
    custEventValue.end = event.end.toLocaleString();
    console.log('check date:', custEventValue);
    this.setState({
      selectedEvent: custEventValue,
    });
  ...
  createSelectbox(data: any, value: any, label: any, uniqueKey: any) {
    ...
    for (let i = 0; i < data.length; i++) {
      let option = data[i];
      retData.push(
        <option key={option[value] + '-' + uniqueKey} value={option[value]}>
          {option[label]}
        </option>
      );
    }
    ...
    if (selectedBatch) {
      for (let i = 0; i < data.length; i++) {
        ...
        if (option.batch.id === parseInt(selectedBatch, 10)) {
          retData.push(
            <option key={option['id'] + '-subject'} value={option['id']}>
              {option['subjectDesc']}
            </option>
          );
        }
      ...
      for (let i = 0; i < attendanceMaster.length; i++) {
        ...
        if (subject.id === parseInt(selectedSubject, 10)) {
          if (addedTeachers.indexOf(master.teach.teacher.id) === -1) {
            retData.push(
              <option
                value={master.teach.teacher.id}
                key={master.teach.teacher.id + '-teacher'}
              >
                {master.teach.teacher.teacherName} {master.teach.teacher.teacherLastName}
              </option>
            );
    ...
    return (
      <div>
        <div className="gf-form-group row m-b-0 p-l-1">
          <div
            className={`gf-form--grow form-control-container m-b-1 ${
              submitted && !selectedTerm ? 'has-error' : ''
            }`}
          >
            <label className="gf-form-label bg-transparent b-0">Term/Semester</label>
            <select
              className="gf-form-select-box"
              name="selectedTerm"
              value={selectedTerm}
            ...
            {submitted && !selectedTerm && <div className="error">Term is required.</div>}
          </div>
          <div
            className={`gf-form--grow form-control-container m-b-1 ${
              submitted && !selectedBatch ? 'has-error' : ''
            }`}
          >
          ...
          <div
            className={`gf-form--grow form-control-container m-b-1 ${
              submitted && !selectedSection ? 'has-error' : ''
            }`}
          >
        ...
        <div className="gf-form-group row m-b-0 p-l-1"></div>
        ...
        <div className="gf-form-group row p-l-1">
          <div className="gf-form--grow form-control-container cust-width-220">
            <input
              type="button"
              className="btn btn-primary gf-form-control"
                  ...
                  <div className="modal-fwidth">
                    <div className="row">
                      ...
                      <label className="col-sm-6">
                        <b>{selectedEvent.teacher}</b>
                      </label>
                    </div>
                    <div className="row">
                      <label className="col-sm-6">Subject: </label>
                      <label className="col-sm-6">
                        <b>{selectedEvent.subject}</b>
                      </label>
                    </div>
                    <div className="row">
                      ...
                      <label className="col-sm-6">
                        <b>{selectedEvent.sex}</b>
                      </label>
                    </div>
                    <div className="row">
                      <label className="col-sm-6">Start Date and Time: </label>
                      <label className="col-sm-6">
                        <b>{selectedEvent.start}</b>
                      </label>
                    </div>
                    <div className="row">
                      <label className="col-sm-6">End Date and Time: </label>
                      <label className="col-sm-6">
                        <b>{selectedEvent.end}</b>
                      </label>
                    </div>
                  </div>
                  <div className="m-t-1 text-center">
                    <button
                      className="btn btn-primary border-bottom m-r-1"
